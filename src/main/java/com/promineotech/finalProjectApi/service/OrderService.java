...
import com.promineotech.finalProjectApi.repository.CustomerRepository;
import com.promineotech.finalProjectApi.repository.OrderRepository;
import com.promineotech.finalProjectApi.repository.ProductRepository;
import com.promineotech.finalProjectApi.service.OrderService;
import com.promineotech.finalProjectApi.util.SustainabilityProceeds;
import com.promineotech.finalProjectApi.util.OrderStatus;
...
public class OrderService {
	...
	private Order initializeNewOrder(Set<Long> productIds, Customer customer) {
		...
		order.setInvoiceAmount(calculateOrderTotal(order.getProducts(), customer.getProceeds()));
	...
	private double calculateOrderTotal(Set<Product> products, SustainabilityProceeds proceeds) {
		double total = 0;
		for (Product product : products) {
			total += product.getPrice();
		}
		return total + total * proceeds.getProceed();
 	}
}